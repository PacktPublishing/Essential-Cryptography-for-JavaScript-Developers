...
// This receives an encrypted message (message) using ECIES (using the format defined in ecies-client.js) as well as the client's public key (publicKey) and the ID of the private key the server (serverKeyId) generated in the /public-key step
// It decrypts the message received and shows it in the console
// This request expects input in JSON format in the POST body
app.post('/message', express.json(), (req, res) => {
    // Get the message from the input; this is encoded as base64, so it has to be decoded
    if (!req.body.message) {
    ...
    const decrypted = aesDecrypt(symmetricKey, message.slice(16))
...
/** Example symmetric decryption function from aes-256-gcm */
